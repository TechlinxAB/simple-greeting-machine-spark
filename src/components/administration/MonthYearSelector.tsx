  ...
  return <div className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:items-center sm:space-x-2">
      ...
      <div className={`flex space-x-2 ${isAllSelected ? 'opacity-50' : ''}`}>
        <div className="flex items-center space-x-1">
          ...
          <Select value={selectedMonth.toString()} onValueChange={value => onMonthYearChange(parseInt(value), selectedYear)} disabled={isAllSelected}>
          ...
          <Select value={selectedYear.toString()} onValueChange={value => onMonthYearChange(selectedMonth, parseInt(value))} disabled={isAllSelected}>
        ...
        <Button variant="outline" size="sm" onClick={handleCurrentMonth} disabled={isAllSelected}>
          Current
        </Button>
      </div>
    </div>;
